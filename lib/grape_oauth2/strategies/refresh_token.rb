    ...
    class RefreshToken < Base
      class << self
        def process(request, &authenticator)
          ...
          refresh_token = config._access_token_class.authenticate(request.refresh_token, type: :refresh_token)
          request.invalid_grant! if refresh_token.nil?
          request.unauthorized_client! if refresh_token && refresh_token.client != client

          token = config._access_token_class.create_for(client, refresh_token.resource_owner)
